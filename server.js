    ...
    ws.on("message", (message) => {
        try {
            ...
            if (cmd === 1003 && gBB && d1 && d2 && d3) {
                ...
                console.log(`[🎲] #${currentSessionId} | [${d1},${d2},${d3}] = ${total} | ${result}`);
...
// ========== TỰ ĐỘNG CHỐNG ĐỨNG - KIỂM TRA MỖI 7 GIÂY ==========
setInterval(() => {
    const now = Date.now();
    if (now - lastDataTime > 30000) {
        console.log("[🔧] Phát hiện đứng 30s - Tự động reconnect...");
        lastDataTime = now;
        if (ws) {
...
app.get("/api/health", (req, res) => {
    res.json({
        ...
        last_data: new Date(lastDataTime).toISOString()
    });
});

app.get("/", (req, res) => {
    res.json({
        name: "SunWin API - Phuc Six",
        endpoints: {
            current: "/api/ditmemaysun",
            history: "/api/sunwin/history",
            stats: "/api/stats",
            health: "/api/health"
        },
        total_sessions: patternHistory.length,
        websocket: ws ? (ws.readyState === WebSocket.OPEN ? "connected" : "disconnected") : "not initialized"
    });
});
...
app.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(`🔧 Auto-fix: Kiểm tra mỗi 7s, reconnect nếu đứng 30s\n`);
    connectWebSocket();
});        1,
        "MiniGame",
        "GM_apivopnhaan",
